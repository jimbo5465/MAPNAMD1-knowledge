...
from handlers.prompt_cleanup import (
    track_prompt,
    delete_tracked,
    prompt_reply,
)
...
async def kn_promote_mode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
...
async def kn_reporter_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if name is None:
            await prompt_reply(update.message, context, "❌ نام نامعتبر است. دوباره وارد کنید:")
...
async def kn_reporter_title(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if value is None:
        await prompt_reply(update.message, context, "❌ سمت نامعتبر است. دوباره وارد کنید:")
        return KN_REPORTER_TITLE
...
async def kn_description(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await prompt_reply(update.message, context, "🔄 در حال استخراج فیلدها با هوش مصنوعی...")
    ...
    if summary:
        await prompt_reply(update.message, context, "\n".join(summary))
    ...
    if classification.get("conflict"):
        ...
        await prompt_reply(update.message, context, msg,
...
async def kn_type_switch(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await prompt_reply(query.message, context, "\n".join(summary))
...
_kn_photo_pending: dict = {}  # user_id -> {"count": n, "_task": Task}
...
async def _flush_kn_photo_notice(msg, context, user_id: int) -> None:
    ...
    # خلاصهٔ قبلی حذف و جدید جایگزین شود
...
async def kn_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not value:
        await prompt_reply(update.message, context, "❌ تاریخ خالی است. دوباره وارد کنید:")
        return KN_DATE
    ok, _err = validate_jalali_date_str(value)
    if not ok:
        await prompt_reply(update.message, context, "❌ تاریخ نامعتبر است. مثال: ۱۴۰۴/۰۵/۱۲")
        return KN_DATE
...
@require_registration
async def kn_cal_open(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
@require_registration
async def kn_cal_nav(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
@require_registration
async def kn_cal_none(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
@require_registration
async def kn_cal_pick(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
def _ensure_knowledge_entry(context, telegram_id: int | None, user: dict | None = None) -> int:
    """رکورد draft را در صورت نبود میسازد و id آن را برمیگرداند."""
    kid = context.user_data.get(_KEY_ENTRY_ID)
    if kid:
        return kid
    user_row = user or (get_user_by_telegram_id(telegram_id) if telegram_id else None)
    kid = models.add_knowledge_entry(
        telegram_id=(user_row or {}).get("telegram_id", telegram_id or 0),
        knowledge_type=context.user_data.get("kn_type"),
...
async def _go_to_org_menu(update, context) -> int:
    ...
    if user is None:
        target = getattr(update, "message", None) or (
            update.callback_query.message if update.callback_query else None
        )
        await prompt_reply(target, context, "⛔ حساب شما ثبت نشده است. لطفاً /start بزنید.")
        return ConversationHandler.END
...
async def _save_and_preview(msg, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        knowledge_id = _ensure_knowledge_entry(context, telegram_id, user)
...
async def kn_interview_loop_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if not user_text:
            await prompt_reply(update.message, context, "❌ متن خالی است. پاسخ خود را بنویسید:")
            return KN_INTERVIEW_LOOP
...
async def _final_assemble_and_preview(
    ...
    try:
        ...
        await prompt_reply(msg, context, "🔄 در حال ساخت فرم نهایی DANA...")
        knowledge_type = context.user_data["kn_type"]
        fields = context.user_data.get("kn_fields") or {}
        ...
        if org_notify.get("_project_overridden"):
            ...
            # فلگ را پاک کن تا در دفعات بعد تکرار نشود
...
async def kn_org_project(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def kn_org_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if not text:
            await prompt_reply(update.message, context, "❌ متن خالی است. دوباره بنویسید یا /skip:")
            return KN_ORG_META
...
async def kn_tree_pick_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
