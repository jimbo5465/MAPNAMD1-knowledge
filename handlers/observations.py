...
_attach_pending: dict = {}  # user_id -> {"photo": n, "file": n, "_task": Task}
...
async def _flush_attachments_notice(msg, context, user_id: int) -> None:
    ...
    # خلاصهٔ قبلی حذف و خلاصهٔ جدید جایگزین شود
    await delete_tracked(context, "_attach_summary")
    try:
...
@require_registration
async def obs_attachment_send(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        if mime.startswith("image"):
            with open(path, "rb") as fh:
                await query.message.reply_photo(photo=fh, caption=name)
        else:
            with open(path, "rb") as fh:
                await query.message.reply_document(document=fh, filename=name, caption=name)
    except Exception:
