# ربات دانش سازمانی MAPNAMD1-knowledge

ربات **تلگرام و بله** برای **ثبت، مدیریت و جستجوی دانش سازمانی و مشاهدات میدانی** — ویژهٔ واحد MAPNAMD1.

این ربات به کاربران اجازه می‌دهد:
- **مشاهده** ثبت کنند (متن، ویس، عکس، فایل) با عنوان، هشتگ و تاریخ
- **دانش/تجربه سازمانی** را ثبت و ساختارمند کنند (دستی یا مصاحبه با هوش مصنوعی)
- در مشاهدات و دانش‌ها **جستجو** کنند (متن، هشتگ، تاریخ جلالی روز/ماه)
- با **یک حساب واحد** هم از تلگرام و هم از بله استفاده کنند — داده‌ها کاملاً مشترک‌اند

---

## امکانات اصلی

| بخش | توضیح |
|---|---|
| 📓 ثبت مشاهده | ثبت سریع مشاهدهٔ میدانی با متن/ویس + عنوان + هشتگ + تاریخ + پیوست (عکس/PDF) |
| 📝 ثبت دانش | ثبت دانش/تجربهٔ سازمانی — دستی یا مصاحبهٔ هوشمند با AI |
| 🗂️ بایگانی و جستجو | مرور و جستجوی مشاهدات و دانش‌های ثبت‌شده |
| 🌐 نسخهٔ وب (مینی‌اپ) | اپ تحت وب داخل بله/تلگرام — جستجوی پیشرفته، خواندن راحت گزارش‌ها، دانلود پیوست‌ها |
| 👤 پروفایل من | مشاهده و ویرایش اطلاعات ثبت‌نام |
| 🔗 حساب دوپلتفرمی | لینک خودکار حساب بله و تلگرام — دانش و مشاهدات مشترک بین هر دو |

---

## بایگانی و جستجو

دکمهٔ «🗂️ بایگانی و جستجو» در منوی اصلی دو زیربخش دارد:

```
🗂️ بایگانی و جستجو
├── 🗂️ بایگانی مشاهدات
│      ├── 📋 نمایش مشاهده‌های من (لیست کامل)
│      └── 🔍 جستجو (متن/عنوان، هشتگ، تاریخ جلالی روز یا ماه)
└── 📚 بایگانی دانش/تجربه سازمانی
       ├── 📋 نمایش دانش‌های من (لیست صفحه‌بندی‌شده)
       └── 🔍 جستجو (عنوان، متن، شمارهٔ دانش KN-...)
```

- لیست دانش‌ها صفحه‌بندی می‌شود (۶ مورد در هر صفحه).
- جزئیات هر دانش: شمارهٔ KN، نوع، وضعیت (پیش‌نویس/ثبت‌شده)، تاریخ، طبقه‌بندی درخت، هشتگ‌ها و متن.
- بایگانی دانش **فقط دانش‌های خود کاربر** را نشان می‌دهد.

---

## حساب دوپلتفرمی (بله + تلگرام)

هر دو نسخهٔ ربات به **یک دیتابیس** وصل‌اند. کاربر می‌تواند از هر پلتفرمی که دلش خواست استفاده کند:

```
کاربر در تلگرام ثبت‌نام می‌کند  →  همان شخص در بله /start می‌زند
                                      ↓
              تطبیق: شماره موبایل + کد پرسنلی
                                      ↓
        «🔗 حساب بله شما به حساب تلگرامی‌تان متصل شد»
                                      ↓
   مشاهده‌ها، پیش‌نویس دانش و پروفایل — همه در هر دو پلتفرم یکسان
```

- شماره با هر فرمتی پذیرفته می‌شود: `+989****7315`، `989155107315`، `09155107315` — همه یکی در نظر گرفته می‌شوند (۱۰ رقم انتهایی ملاک است).
- 🔒 **امنیت:** لینک فقط وقتی انجام می‌شود که **کد پرسنلی** هم با شماره مطابقت داشته باشد؛ یعنی با داشتن صرفِ شمارهٔ کسی نمی‌توان به داده‌هایش دسترسی پیدا کرد.
- اگر شخص قبلاً در هر دو پلتفرم جداگانه ثبت‌نام کرده باشد، در اولین ثبت‌نام مجدد رکوردها **ادغام** می‌شوند.

---

## ثبت مشاهده (فلوی کامل)

```
متن / ویس / عکس
    ↓
✅ ذخیره متن
    ↓
📝 عنوان (اجباری)
    ↓
#️⃣ هشتگ‌ها (اختیاری — دکمه «رد کردن»)
    ↓
📅 تاریخ (اختیاری — پیش‌فرض امروز، فرمت 1402/12/15)
    ↓
✅ خلاصهٔ کامل مشاهده
    ↓
🖼️📎 پیوست (عکس/PDF/فایل — اختیاری)
```

### ویس
- صدای فارسی کاربر به متن تبدیل می‌شود (Groq whisper-large-v3-turbo)
- متن تشخیص‌داده‌شده نمایش داده می‌شود
- کاربر می‌تواند **اصلاح کند** (جایگزین یا افزودن به متن قبلی) یا **تأیید کند**
- در حالت «افزودن مطلب به مشاهدهٔ موجود»، ویس نیز به همان مشاهده الحاق می‌شود (نه مشاهدهٔ جدید)

### ذخیره‌سازی عکس
- سایز **متوسط** تلگرام (~۸۰۰px) به‌جای بزرگ‌ترین سایز — برای صرفه‌جویی در فضای دیسک
- فایل‌ها در `media/obs_attachments/<obs_id>/` ذخیره می‌شوند

---

## جستجو در مشاهدات

| روش | ورودی کاربر | مثال |
|---|---|---|
| 🔍 عنوان/متن | کلمهٔ کلیدی | `شیر اطمینان` |
| #️⃣ هشتگ | نام هشتگ (بدون #) | `نقص فنی` |
| 📅 تاریخ روز | تاریخ جلالی کامل | `1402/12/15` |
| 📅 ماه کامل | فقط سال و ماه | `1402/12` (جستجوی کل ماه) |

---

## ثبت دانش سازمانی

دو روش:

1. **✍️ ثبت دستی** — کاربر متن شرح را می‌فرستد، AI فیلدهای ساختارمند را استخراج می‌کند (عنوان، درس‌آموخته، راه‌حل و...) و کاربر تأیید/اصلاح می‌کند.
2. **🎙️ مصاحبه با AI** — ربات سؤال‌های متنی بر اساس نوع دانش می‌پرسد تا اطلاعات کامل شود.

پس از تکمیل، پیش‌نویس **DANA** (سند دانش سازمانی) ساخته می‌شود و خروجی PDF/Word قابل دریافت است.

### قفل «در حال پردازش»
هنگامی که AI مشغول است (استخراج فیلد، مصاحبه، ساخت فرم)، هر پیام/دکمهٔ اضافی کاربر با پیام
«⏳ هوش مصنوعی در حال بررسی است...» پاسخ داده می‌شود — نه خطا.

---

## ساختار پروژه

```
MAPNAMD1-knowledge/
├── main.py                    # نقطهٔ ورود ربات تلگرام
├── main_bale.py               # نقطهٔ ورود ربات بله
├── config.py                  # تنظیمات و متغیرهای محیطی
├── db/                        # دیتابیس مشترک دو پلتفرم
│   ├── init.py                # ساخت جداول و migration
│   ├── models.py              # CRUD + لینک حساب‌ها (register_or_link_user)
│   └── phone_utils.py         # نرمال‌سازی شماره موبایل
├── engine/
│   ├── knowledge_ai.py        # استخراج فیلدها با AI
│   ├── knowledge_interview.py # مصاحبهٔ هوشمند + ساخت فرم نهایی
│   ├── knowledge_draft.py     # ساخت گزارش DANA
│   ├── knowledge_render.py    # خروجی PDF/Word
│   ├── knowledge_tree.py      # درخت دانش سازمانی
│   └── knowledge_numbering.py # شماره‌گذاری دانش
├── handlers/                  # لایهٔ رابط تلگرام
│   ├── registration.py        # ثبت‌نام و پروفایل (+ لینک حساب بله)
│   ├── observations.py        # مشاهدات میدانی + جستجو
│   ├── knowledge.py           # ثبت دانش + بایگانی و جستجوی دانش
│   ├── archive.py             # زیرمنوی «بایگانی و جستجو»
│   ├── keyboards.py           # کیبوردها
│   ├── auth.py                # احراز هویت
│   ├── jalali_calendar.py     # تقویم جلالی تعاملی
│   └── prompt_cleanup.py      # حذف پرامپت قبلی ربات
├── bale_app/                  # لایهٔ رابط بله (فریم‌ورک سبک سفارشی)
│   ├── framework.py           # Update/Context/Dispatcher شبیه‌ساز تلگرام روی بله
│   └── ...                    # همان ساختار handlers/
├── webapp/                    # مینی‌اپ وب (FastAPI + SPA)
│   ├── auth.py                # اعتبارسنجی initData + توکن نشست
│   ├── main.py                # API endpoints
│   └── static/                # فرانت‌اند vanilla JS
├── utils/
│   ├── busy_lock.py           # قفل «در حال پردازش» AI
│   ├── dates.py               # تبدیل تاریخ جلالی/میلادی
│   ├── jalali_calendar_core.py # هستهٔ محاسبات تقویم
│   ├── text_normalizer.py     # تطبیق متون با difflib
│   └── validators.py          # اعتبارسنجی
├── scripts/
│   ├── backup_db.py           # بکاپ روزانه دیتابیس (cron 3:30)
│   ├── eval_normalizer.py     # ارزیابی normalizer
│   └── gen_intro_docx.py      # تولید سند معرفی
├── deploy/nginx/              # کانفیگ nginx وب‌اپ
├── data/                      # [runtime] دیتابیس SQLite — مشترک دو ربات
├── media/                     # [runtime] عکس‌ها و فایل‌ها
└── logs/                      # [runtime] لاگ‌ها
```

---

## تکنولوژی

- **Python 3.11**
- **python-telegram-bot 20.7** (نسخهٔ تلگرام) + **فریم‌ورک سبک داخلی** برای بله
- **SQLite** (فایل `data/knowledge.db` — مشترک بین دو پلتفرم)
- **Groq API** — تبدیل صوت به متن (whisper-large-v3-turbo)
- **OpenCode Go / deepseek-v4-flash** — هوش مصنوعی استخراج فیلد و مصاحبه
- **jdatetime** — تاریخ جلالی
- **reportlab / python-docx** — خروجی PDF/Word
- **FastAPI + uvicorn** — وب‌اپ (مینی‌اپ بله/تلگرام)

---

## نصب و اجرا

```bash
# ۱. کلون
git clone https://github.com/jimbo5465/MAPNAMD1-knowledge.git
cd MAPNAMD1-knowledge

# ۲. محیط مجازی
python -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt

# ۳. متغیرهای محیطی
export BOT_TOKEN="توکن ربات تلگرام از BotFather"
export BALE_BOT_TOKEN="توکن ربات بله"
export KNOWLEDGE_AI_API_KEY="کلید OpenCode Go"
export KNOWLEDGE_AI_MODEL="deepseek-v4-flash"
export GROQ_API_KEY="کلید Groq"

# ۴. اجرا (هر پلتفرم جداگانه)
python main.py         # نسخهٔ تلگرام
python main_bale.py    # نسخهٔ بله
```

---

## استقرار با systemd (VPS)

```bash
sudo systemctl enable --now knowledgebot          # نسخهٔ تلگرام (main.py)
sudo systemctl enable --now knowledgebot-bale     # نسخهٔ بله (main_bale.py)
sudo systemctl enable --now knowledgebot-webapp   # وب‌اپ (uvicorn)

sudo systemctl status knowledgebot knowledgebot-bale knowledgebot-webapp
sudo journalctl -u knowledgebot -f                # لاگ زنده
```

---

## مجوز

پروژهٔ داخلی MAPNAMD1 — استفاده برای واحد دانش سازمانی.