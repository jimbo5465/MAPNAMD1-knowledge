"""
هندلر ثبت‌نام کاربر (معرفی) — نسخه بله.
...
(پورت از handlers/registration.py — فقط منبع import تغییر کرده)
"""
...
async def start_registration(update, context) -> int:
...
async def get_phone(update, context) -> int:
...
async def get_personnel_code(update, context) -> int:
    text = (update.message.text or "").strip()
    if not text:
...
async def get_project(update, context) -> int:
    text = (update.message.text or "").strip()
...
async def skip_project(update, context) -> int:
...
async def _ask_position(update, context) -> None:
    """پرسش سمت."""
    text = (
        "✅ ثبت شد.\n\n"
...
async def get_position(update, context) -> int:
    text = (update.message.text or "").strip()
...
async def skip_position(update, context) -> int:
    query = update.callback_query
    await query.answer()
...
async def _finish_registration(update, context) -> None:
    ...
    linked = False
    try:
        # ثبت‌نام جدید یا اتصال به حساب تلگرامیِ همان شخص
        # (تطبیق: شمارهٔ نرمال‌شده + کد پرسنلی — لایهٔ امنیتی)
        _, linked = register_or_link_user(
            platform="bale",
            platform_id=user.id,
            full_name=context.user_data.get("reg_name", user.first_name or "کاربر"),
        ...
        if linked:
            logger.info("حساب بله به حساب موجود متصل شد: %d", user.id)
        else:
            logger.info("کاربر بله ثبت شد: %d (%s)", user.id, context.user_data.get("reg_name"))
    except Exception:
    ...
    if linked:
        text += "\n\n🔗 حساب بله شما به حساب تلگرامی‌تان متصل شد — دانش‌ها و مشاهدات شما در هر دو پلتفرم مشترک است."
    if update.callback_query:
        await update.callback_query.edit_message_text(
            text, parse_mode="Markdown",
            reply_markup=main_menu_keyboard(user.id),
...
async def profile_view(update, context) -> int:
    """نمایش پروفایل کاربر."""
    query = update.callback_query
    await query.answer()
    user = update.effective_user
    ...
    from bale_app.keyboards import profile_keyboard
    text = (
        "👤 *پروفایل شما*\n\n"
...
async def profile_edit_start(update, context) -> int:
    """شروع ویرایش پروفایل."""
    query = update.callback_query
    await query.answer()
...
async def profile_edit_skip(update, context) -> int:
...
async def _finish_edit(update, context) -> None:
